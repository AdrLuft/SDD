...
_Liste tudo que, se for falso, invalida o PRD. Evita decisões fantasiosas na spec._
...
| Conclusão cadastro | Do início do cadastro até “conta utilizável e sessão ativa” (conforme política de confirmação de e-mail) | A definir | ≥ 80% dos inícios | Mensal no piloto |
...
| --- | --- | --- | --- |
| Dono / gestor | Criar sua conta, entrar, recuperar senha; eventualmente criar as primeiras contas da equipe (se o produto permitir) | Média | Conforme piloto |
| Equipe (recepção / instrutores) | Conta própria para **login** no app quando existir algo além da tela de auth | Baixa a média | Diária (após haver app pós-login) |

*Personas e necessidades de **aluno**, **financeiro** e **contador** ficam fora até as features respectivas.*
...
- **Marketplace / multi-tenant B2B complexo** — fora; decisão de “uma organização vs. várias” pode ficar simplificada ou adiada na spec, desde que não bloqueie cadastro/login.

### 3.3 Papéis, permissões e dados sensíveis

- **Quem pode ver / fazer o quê?** Apenas o necessário para **auth**: qualquer usuário autenticado acessa o que o produto expuser **após** o login nesta fase (ex.: placeholder ou home mínima). **Sem** matriz de permissões neste PRD.
- **Dados pessoais / LGPD:** Cadastro implica **e-mail** (e eventualmente nome, se coletado no cadastro). Política de privacidade e termos antes de escalar além de piloto; detalhes de consentimento na spec.
- **Auditoria:** Desejável registrar **criação de conta**, **login bem-sucedido** e **troca de senha / reset** em nível mínimo para suporte e segurança; escopo fino na spec técnica (sem exigir auditoria de módulos que ainda não existem).
...
| --- | --- | --- | --- |
| PRD-RF-001 | P0 | O sistema deve permitir **cadastro** de usuário com dados mínimos acordados (ex.: e-mail + senha) e confirmação de identidade se a política de produto exigir. | Evitar conta duplicada pelo mesmo identificador. |
...
| PRD-RF-004 | P0 | O sistema deve manter **sessão** (ou equivalente) de forma que o usuário permaneça autenticado conforme política de segurança até logout ou expiração. | “Lembrar-me” opcional — decidir na spec. |
...
Para cada fluxo: gatilho → passos → resultado → falhas esperadas.

1. **Fluxo: Cadastro**  
   - **Feliz:** Gatilho “criar conta” → preenche dados → (opcional) confirma e-mail, conforme decisão em aberto → sessão ativa ou tela de login com mensagem de sucesso → área autenticada mínima (ex.: home placeholder).  
   - **Alternativos:** — *(convites, primeiro admin da organização, etc. ficam para features futuras, salvo inclusão explícita na spec desta entrega).*  
...
2. **Fluxo: Login**  
   ...
   - **Erro / vazio / sem permissão:** Credenciais inválidas; conta desativada; mensagem genérica + opção de recuperação; bloqueio temporário após N tentativas (comportamento na spec).

3. **Fluxo: Recuperação de senha**  
   - **Feliz:** Solicita reset → recebe link → define nova senha → login com nova senha.  
...
- **Vazio:** Nenhuma conta ainda — primeiro cadastro deve resultar em usuário utilizável (e política de “primeiro usuário” simplificada, se necessário, na spec).
- **Grande volume:** Muitas contas e logins — limites e performance dos endpoints de auth na spec (sem requisito de listagem de usuários neste PRD, salvo admin futuro).
...
- **Acessibilidade:** Teclado e foco nos três fluxos (cadastro, login, recuperação); alvo WCAG na spec de UI quando aplicável.
...
### 5.2 Referências de design

- **Figma / protótipo:** Não há — definir no repo alvo quando existir.
- **Componentes / design system:** Conforme stack do projeto (documentar em TECH-SPEC-IA.md).
- **Copy / tom de voz:** Profissional, direto, empático em erros; sem culpar o usuário.

### 5.3 Notificações, e-mails e mensagens de erro (se aplicável)
...
- **Opt-in marketing:** fora do escopo dos e-mails transacionais de auth, salvo consentimento explícito em fluxo separado.

---

## 6. Fora de escopo (não objetivos)

_Liste **não** apenas “não faremos”, mas **por quê** (evita reinterpretação)._

1. **Módulos de academia / ERP** (cadastro de alunos, planos de treino, mensalidades, estoque, etc.) — **Motivo:** esta entrega é só **auth**; o restante terá PRD/spec próprios.
...
- **LGPD:** tratamento de dados pessoais com finalidade informada; base legal para cadastro operacional; canal para solicitações do titular quando o produto estiver em produção com dados reais.
- **Termos de uso e política de privacidade:** publicação antes de escalar usuários externos além de piloto controlado.
- **Senha e dados sensíveis:** política mínima de complexidade e armazenamento conforme boas práticas (detalhe técnico fora do PRD).
- **Saúde / aptidão:** fora desta entrega; quando houver módulo de treinos/saúde, tratar em PRD específico.

### 7.3 Analytics e experimentação (se aplicável)

- **Eventos a disparar (sugestão):** `signup_started`, `signup_completed`, `login_success`, `login_failure`, `password_reset_requested`, `password_reset_completed`, `logout` — com propriedades mínimas (sem PII desnecessária).
- **Feature flag:** opcional para **recuperação de senha** ou política de confirmação de e-mail até estabilizar; nomes na spec.
...
| Confirmação de e-mail mal definida trava UX | M | M | Decidir cedo na spec (§8.3); default explícito |
...
### 9.3 Plano de rollback (se aplicável)
...
Esta seção liga o PRD à cadeia **ideia → spec → tarefas → implementação → validação**.
...
- [ ] Escopo **só auth** e prioridades P0 (PRD-RF-001…005) acordados com stakeholders
