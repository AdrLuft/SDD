...
O marcador desta instrução é:  SDD1️⃣
...
- **Padrões e normas do repositório**: Identifique padrões de código, arquitetura e práticas de desenvolvimento a partir de:
  ...
  - Arquivos de configuração (package.json, Cargo.toml, pyproject.toml, etc.)
  - Estrutura e convenções de nomenclatura do código existente
  - Padrões de testes e garantia de qualidade
  - Convenções de mensagem de commit e fluxos de desenvolvimento
...
- Implementar um sistema de autenticação completo do zero
...
- Adicionar um único `console.log` para depuração
- Mudar a cor de um botão no CSS
...
- Adicionar uma nova flag de CLI com validação e texto de ajuda
- Implementar um único endpoint de API com validação de request/response
- Refatorar um módulo mantendo compatibilidade retroativa
- Adicionar um novo componente integrado ao gerenciamento de estado existente
...
- Só avance para a Etapa 5 quando:
  ...
  - Houver detalhe suficiente para preencher todas as seções da spec (histórias de usuário, unidades demonstráveis com requisitos funcionais, etc.).
...
- Exemplo: `Captura de tela: saída de `--help` demonstra que o novo comando existe`
- Exemplo: `CLI: `comando --flag` retorna saída esperada demonstra que a feature funciona`
...
**Artefatos de prova:**
- [Tipo de artefato]: [descrição] demonstra [o que comprova]
- Exemplo: `Teste: MyFeature.test.ts passa demonstra implementação do requisito`
- Exemplo: `PDF do pedido: PDF baixado de https://example.com/order-submitted mostra fluxo concluído demonstra funcionalidade ponta a ponta`
...
- Padrões de código e guias de estilo do repositório
- Padrões arquiteturais e organização de arquivos
- Convenções de testes e práticas de garantia de qualidade
...
- Inclua artefatos de prova para cada unidade de trabalho que demonstrem o que será mostrado
- Siga em todos os requisitos os padrões e práticas do repositório identificados
